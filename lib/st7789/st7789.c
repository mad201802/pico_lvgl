...
void st7789_init(const struct st7789_config* config, uint16_t width, uint16_t height)
{
    ...
    if (st7789_cfg.use_dma) {
        dma_channel = dma_claim_unused_channel(true);
        dma_config = dma_channel_get_default_config(dma_channel);
        
        // Configure DMA for SPI transfers
        channel_config_set_transfer_data_size(&dma_config, DMA_SIZE_16);
        channel_config_set_dreq(&dma_config, spi_get_dreq(st7789_cfg.spi, true));
...
void st7789_write(const void* data, size_t len)
{
    ...
    // Use DMA if available, otherwise fall back to blocking write
    if (st7789_cfg.use_dma && dma_channel >= 0) {
        ...
        dma_channel_configure(
            dma_channel,
            &dma_config,
            &spi_get_hw(st7789_cfg.spi)->dr,  // Write address (SPI data register)
            data,                              // Read address (source buffer)
            len / 2,                          // Transfer count (16-bit words)
...
{
    // Reset data mode flag to ensure proper command sequence
    // This is critical when setting cursor for multiple flush operations
    st7789_data_mode = false;
